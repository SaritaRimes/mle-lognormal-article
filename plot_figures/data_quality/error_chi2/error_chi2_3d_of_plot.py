...
error_of = np.array(import_error_data_quality_separately_crossvalidation(occupancy, iteration, number_iterations,
                                                                         method))
chi2_of = np.array(import_chi2_data_quality_separately_crossvalidation(occupancy, iteration, number_iterations,
                                                                       method))

# Making the interpolation for 3D graph
data, x_e, y_e = np.histogram2d(error_of, chi2_of, bins=30, density=False)
z = interpn((x_e[:-1], y_e[:-1]), data, np.vstack([error_of, chi2_of]).T, method='splinef2d',
            bounds_error=False)
z[np.where(np.isnan(z))] = 0.0
idx = z.argsort()
error_of_sort, chi2_of_sort, z_sort = error_of[idx], chi2_of[idx], z[idx]
...
plt.scatter(error_of_sort, chi2_of_sort, c=z_sort, s=2, zorder=3, norm=norm)
