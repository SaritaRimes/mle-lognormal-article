...
# Importing the data
error_cof = np.array(import_error_data_quality_separately_crossvalidation(occupancy, iteration, number_iterations,
                                                                          method))
chi2_cof = np.array(import_chi2_data_quality_separately_crossvalidation(occupancy, iteration, number_iterations,
                                                                        method))

# Making the interpolation for 3D graph
data, x_e, y_e = np.histogram2d(error_cof, chi2_cof, bins=30, density=False)
z = interpn((x_e[:-1], y_e[:-1]), data, np.vstack([error_cof, chi2_cof]).T, method='splinef2d',
            bounds_error=False)
z[np.where(np.isnan(z))] = 0.0
idx = z.argsort()
error_cof_sort, chi2_cof_sort, z_sort = error_cof[idx], chi2_cof[idx], z[idx]
...
plt.scatter(error_cof_sort, chi2_cof_sort, c=z_sort, s=2, zorder=3, norm=norm)
