...
# locale.setlocale(locale.LC_NUMERIC, locale.getlocale()[0])  # puts comma in the place of points
...
method_fig = {'mle_gaussiano': 'gauss', 'of': 'of', 'cof': 'cof', 'mle_lognormal': 'logn'}

# Importing the data
error_logn = np.array(import_error_data_quality_separately_crossvalidation(occupancy, iteration, number_iterations,
                                                                           method))
chi2_logn = np.array(import_chi2_data_quality_separately_crossvalidation(occupancy, iteration, number_iterations,
                                                                         method))

# Making the interpolation for 3D graph
data, x_e, y_e = np.histogram2d(error_logn, chi2_logn, bins=30, density=False)
z = interpn((x_e[:-1], y_e[:-1]), data, np.vstack([error_logn, chi2_logn]).T, method='splinef2d',
            bounds_error=False)
z[np.where(np.isnan(z))] = 0.0
idx = z.argsort()
error_logn_sort, chi2_logn_sort, z_sort = error_logn[idx], chi2_logn[idx], z[idx]
...
plt.scatter(error_logn_sort, chi2_logn_sort, c=z_sort, s=2, zorder=3, norm=norm)
plt.xlabel('Error (ADC counts)', fontsize=legends_factor * 10)
plt.ylabel('$\u03C7^2$', fontsize=legends_factor * 10)
...
# plt.savefig(f'D:/Documentos/UERJ/Doutorado/ArtigoIEEE/Figuras/erro_chi2_{method_fig[method]}'
