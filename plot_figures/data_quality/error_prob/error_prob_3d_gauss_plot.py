...
error_gauss = np.array(import_error_data_quality_separately_crossvalidation(occupancy, iteration, number_iterations,
                                                                            method))
prob_gauss = np.array(import_prob_data_quality_separately_crossvalidation(occupancy, iteration, number_iterations,
                                                                          method))

# Making the interpolation for 3D graph
data, x_e, y_e = np.histogram2d(error_gauss, prob_gauss, bins=160, density=False)
z = interpn((x_e[:-1], y_e[:-1]), data, np.vstack([error_gauss, prob_gauss]).T, method='splinef2d',
            bounds_error=False)
z[np.where(np.isnan(z))] = 0.0
idx = z.argsort()
error_gauss_sort, prob_gauss_sort, z_sort = error_gauss[idx], prob_gauss[idx], z[idx]
...
plt.scatter(error_gauss_sort, prob_gauss_sort, c=z_sort, s=2, zorder=3, norm=norm)
