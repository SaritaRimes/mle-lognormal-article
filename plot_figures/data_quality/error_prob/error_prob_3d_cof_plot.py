...
# Importing the data
error_cof = np.array(import_error_data_quality_separately_crossvalidation(occupancy, iterarion, number_iterations,
                                                                          method))
prob_cof = np.array(import_prob_data_quality_separately_crossvalidation(occupancy, iterarion, number_iterations,
                                                                        method))

# Making the interpolation for 3D graph
data, x_e, y_e = np.histogram2d(error_cof, prob_cof, bins=160, density=False)
z = interpn((x_e[:-1], y_e[:-1]), data, np.vstack([error_cof, prob_cof]).T, method='splinef2d',
            bounds_error=False)
z[np.where(np.isnan(z))] = 0.0
idx = z.argsort()
error_cof_sort, prob_cof_sort, z_sort = error_cof[idx], prob_cof[idx], z[idx]
...
plt.scatter(error_cof_sort, prob_cof_sort, c=z_sort, s=2, zorder=3, norm=norm)
